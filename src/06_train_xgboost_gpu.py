...
CPU_METRICS_PATH = METRICS_DIR / "xgboost_cpu_metrics.csv"
LOGISTIC_METRICS_PATH = METRICS_DIR / "logistic_baseline_metrics.csv"

MEDIUM_MODEL_PATH = MODELS_DIR / "xgboost_gpu_medium.json"
HEAVY_MODEL_PATH = MODELS_DIR / "xgboost_gpu_heavy.json"
METRICS_PATH = METRICS_DIR / "xgboost_gpu_metrics.csv"
THRESHOLDS_PATH = METRICS_DIR / "xgboost_gpu_thresholds.csv"
COMPARISON_PATH = METRICS_DIR / "xgboost_cpu_gpu_comparison.csv"
GPU_DIAGNOSTIC_PATH = METRICS_DIR / "xgboost_gpu_diagnostic.txt"
PR_CURVE_PATH = FIGURES_DIR / "xgboost_gpu_pr_curve.png"
ROC_CURVE_PATH = FIGURES_DIR / "xgboost_gpu_roc_curve.png"
FEATURE_IMPORTANCE_PATH = FIGURES_DIR / "xgboost_gpu_feature_importance.png"
...
def split_xy(df: pd.DataFrame, feature_columns: list[str]) -> tuple[pd.DataFrame, pd.Series]:
    return df[feature_columns].astype(np.float32), df["label"].astype(int)
...
def model_configs(imbalance: dict[str, float]) -> list[dict]:
    ...
    return [
        {
            "model_name": "xgboost_gpu_medium_spw_1",
            "model_family": "medium",
            "cpu_match_name": "xgboost_cpu_medium_spw_1",
            "params": {**medium, "scale_pos_weight": 1.0},
        },
        {
            "model_name": "xgboost_gpu_medium_spw_sqrt",
            "model_family": "medium",
            "cpu_match_name": "xgboost_cpu_medium_spw_sqrt",
            "params": {**medium, "scale_pos_weight": sqrt_neg_pos_ratio},
        },
        {
            "model_name": "xgboost_gpu_medium_spw_ratio",
            "model_family": "medium",
            "cpu_match_name": "xgboost_cpu_medium_spw_ratio",
            "params": {**medium, "scale_pos_weight": neg_pos_ratio},
        },
        {
            "model_name": "xgboost_gpu_medium_spw_ratio_mds1",
            "model_family": "medium",
            "cpu_match_name": "xgboost_cpu_medium_spw_ratio_mds1",
            "params": {
                **medium,
                "scale_pos_weight": neg_pos_ratio,
                "max_delta_step": 1,
        ...
        {
            "model_name": "xgboost_gpu_heavy_spw_ratio_mds1",
            "model_family": "heavy",
            "cpu_match_name": "xgboost_cpu_heavy_spw_ratio_mds1",
            "params": heavy,
        },
    ]


def write_gpu_diagnostic(message: str) -> None:
    METRICS_DIR.mkdir(parents=True, exist_ok=True)
    GPU_DIAGNOSTIC_PATH.write_text(message, encoding="utf-8")


def assert_no_cuda_fallback(warning_messages: list[str]) -> None:
    fallback_markers = [
        "no visible gpu",
    ...
    for message in warning_messages:
        ...
        if any(marker in lowered for marker in fallback_markers):
            raise RuntimeError(f"XGBoost CUDA warning indicates GPU is unavailable: {message}")


def gpu_sanity_check(x_train: pd.DataFrame, y_train: pd.Series) -> None:
    log("[INFO] Running small CUDA sanity check before full GPU training.")
    subset_size = min(10_000, len(x_train))
    ...
    try:
        with warnings.catch_warnings(record=True) as caught_warnings:
            ...
            sanity_model.fit(x_train.iloc[:subset_size], y_train.iloc[:subset_size], verbose=False)
        warning_messages = [str(warning.message) for warning in caught_warnings]
        assert_no_cuda_fallback(warning_messages)
        config = sanity_model.get_booster().save_config().lower()
        if '"device":"cuda' not in config and '"device":"gpu' not in config:
            raise RuntimeError("Booster config does not indicate CUDA/GPU device use.")
        log("[OK] GPU sanity check passed; CUDA training is available.")
    except Exception as exc:
        message = (
            "GPU XGBoost sanity check failed. Full GPU training was not run.\n"
            f"Reason: {exc}\n"
        )
        write_gpu_diagnostic(message)
        log(f"[ERROR] {message.strip()}")
        raise
...
def plot_curves(
    ...
    ax.set_title(f"GPU XGBoost Precision-Recall Curve ({best_model_name})")
    ...
    ax.set_title(f"GPU XGBoost ROC Curve ({best_model_name})")
...
def plot_feature_importance(best_model: XGBClassifier) -> None:
    ...
    ax.set_title("GPU XGBoost Top 20 Features by Gain")
    ax.set_xlabel("Gain")
...
def load_first_rows(metrics_path: Path, model_prefix: str) -> pd.DataFrame:
...
def build_cpu_gpu_comparison(
    gpu_metrics: pd.DataFrame,
    cpu_metrics_path: Path,
    logistic_metrics_path: Path,
    configs: list[dict],
    best_gpu_model_name: str,
) -> pd.DataFrame:
    rows = []
    cpu_metrics = load_first_rows(cpu_metrics_path, "xgboost_cpu")
    gpu_first = gpu_metrics.drop_duplicates(["model_name", "split"]).copy()

    for config in configs:
        gpu_name = config["model_name"]
        cpu_name = config["cpu_match_name"]
        gpu_val = gpu_first.loc[
            gpu_first["model_name"].eq(gpu_name) & gpu_first["split"].eq("validation")
        ]
        gpu_test = gpu_first.loc[
            gpu_first["model_name"].eq(gpu_name) & gpu_first["split"].eq("test")
        ]
        cpu_val = cpu_metrics.loc[
            cpu_metrics["model_name"].eq(cpu_name) & cpu_metrics["split"].eq("validation")
        ]
        cpu_test = cpu_metrics.loc[
            cpu_metrics["model_name"].eq(cpu_name) & cpu_metrics["split"].eq("test")
        ]
        if gpu_val.empty or gpu_test.empty:
            continue
        row = {
            "comparison_type": "cpu_gpu_matching_config",
            "config": gpu_name.replace("xgboost_gpu_", ""),
            "cpu_model_name": cpu_name,
            "gpu_model_name": gpu_name,
            "gpu_validation_pr_auc": float(gpu_val.iloc[0]["pr_auc"]),
            "gpu_test_pr_auc": float(gpu_test.iloc[0]["pr_auc"]),
            "gpu_test_roc_auc": float(gpu_test.iloc[0]["roc_auc"]),
            "gpu_training_time_seconds": float(gpu_val.iloc[0]["training_time_seconds"]),
        }
        if not cpu_val.empty and not cpu_test.empty:
            row.update(
                {
                    "cpu_validation_pr_auc": float(cpu_val.iloc[0]["pr_auc"]),
                    "cpu_test_pr_auc": float(cpu_test.iloc[0]["pr_auc"]),
                    "cpu_test_roc_auc": float(cpu_test.iloc[0]["roc_auc"]),
                    "cpu_training_time_seconds": float(cpu_val.iloc[0]["training_time_seconds"]),
                    "gpu_cpu_speedup": float(cpu_val.iloc[0]["training_time_seconds"])
                    / float(gpu_val.iloc[0]["training_time_seconds"]),
                }
            )
        rows.append(row)

    logistic = load_first_rows(logistic_metrics_path, "logistic")
    if not logistic.empty:
        logistic_val = logistic.loc[logistic["split"].eq("validation")].iloc[0]
        logistic_test = logistic.loc[logistic["split"].eq("test")].iloc[0]
        best_gpu_val = gpu_first.loc[
            gpu_first["model_name"].eq(best_gpu_model_name) & gpu_first["split"].eq("validation")
        ].iloc[0]
        best_gpu_test = gpu_first.loc[
            gpu_first["model_name"].eq(best_gpu_model_name) & gpu_first["split"].eq("test")
        ].iloc[0]
        rows.append(
            {
                "comparison_type": "best_model_quality",
                "config": "logistic_vs_best_gpu",
                "cpu_model_name": "",
                "gpu_model_name": best_gpu_model_name,
                "logistic_validation_pr_auc": float(logistic_val["pr_auc"]),
                "logistic_test_pr_auc": float(logistic_test["pr_auc"]),
                "logistic_test_roc_auc": float(logistic_test["roc_auc"]),
                "logistic_test_precision_at_100": float(logistic_test["precision_at_100"]),
                "logistic_test_recall_at_100": float(logistic_test["recall_at_100"]),
                "logistic_test_precision_at_500": float(logistic_test["precision_at_500"]),
                "logistic_test_recall_at_500": float(logistic_test["recall_at_500"]),
                "gpu_validation_pr_auc": float(best_gpu_val["pr_auc"]),
                "gpu_test_pr_auc": float(best_gpu_test["pr_auc"]),
                "gpu_test_roc_auc": float(best_gpu_test["roc_auc"]),
                "gpu_test_precision_at_100": float(best_gpu_test["precision_at_100"]),
                "gpu_test_recall_at_100": float(best_gpu_test["recall_at_100"]),
                "gpu_test_precision_at_500": float(best_gpu_test["precision_at_500"]),
                "gpu_test_recall_at_500": float(best_gpu_test["recall_at_500"]),
            }
        )
...
def save_outputs(
    ...
    heavy_model: XGBClassifier | None,
    ...
    if heavy_model is not None:
        expected_paths.append(HEAVY_MODEL_PATH)
...
def print_comparison_summary(best_gpu_model_name: str, gpu_metrics: pd.DataFrame) -> None:
    gpu_first = gpu_metrics.drop_duplicates(["model_name", "split"])
    best_gpu_val = gpu_first.loc[
        gpu_first["model_name"].eq(best_gpu_model_name) & gpu_first["split"].eq("validation")
    ].iloc[0]
    best_gpu_test = gpu_first.loc[
    ...
    log("\n" + "=" * 80)
    log("GPU XGBoost summary")
    log("=" * 80)
    log(f"[INFO] Best GPU model by validation PR-AUC: {best_gpu_model_name}")
    log(f"[INFO] Best GPU validation PR-AUC: {best_gpu_val['pr_auc']:.6f}")
    log(f"[INFO] Best GPU validation ROC-AUC: {best_gpu_val['roc_auc']:.6f}")
    log(f"[INFO] Best GPU test PR-AUC: {best_gpu_test['pr_auc']:.6f}")
    log(f"[INFO] Best GPU test ROC-AUC: {best_gpu_test['roc_auc']:.6f}")
    for k in [100, 500]:
        log(
            f"[INFO] Best GPU test Precision@{k}: {best_gpu_test[f'precision_at_{k}']:.6f}; "
            f"Recall@{k}: {best_gpu_test[f'recall_at_{k}']:.6f}"
        )


def main() -> None:
    log("=" * 80)
    log("Training GPU XGBoost models")
    log("=" * 80)
    ...
    gpu_sanity_check(x_by_split["train"], y_by_split["train"])
    ...
    best_medium_name = str(
        validation_summary.loc[validation_summary["model_family"].eq("medium")].iloc[0][
            "model_name"
        ]
    )
    heavy_name = "xgboost_gpu_heavy_spw_ratio_mds1"

    plot_curves(best_model_name, y_by_split, scores_by_model)
    plot_feature_importance(models[best_model_name])
    comparison = build_cpu_gpu_comparison(
        metrics,
        CPU_METRICS_PATH,
    ...
    save_outputs(
        ...
        heavy_model=models.get(heavy_name),
        metrics=metrics,
        thresholds=thresholds,
        comparison=comparison,
    )

    print_comparison_summary(best_model_name, metrics)
    ...
    if not comparison.empty:
        log("\n" + "=" * 80)
        log("CPU/GPU comparison")
        log("=" * 80)
        match_rows = comparison.loc[
            comparison["comparison_type"].eq("cpu_gpu_matching_config")
        ]
        for row in match_rows.to_dict("records"):
            if pd.notna(row.get("cpu_training_time_seconds", np.nan)):
                log(
                    f"[INFO] {row['config']}: CPU {row['cpu_training_time_seconds']:.2f}s, "
                    f"GPU {row['gpu_training_time_seconds']:.2f}s, "
                    f"speedup {row['gpu_cpu_speedup']:.2f}x"
                )
